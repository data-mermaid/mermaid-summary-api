# Generated by Django 2.2.3 on 2019-09-10 01:56
...
LEFT JOIN (
    ...
    FROM (
        ...
        FROM (
            SELECT site_id, management_id, sample_date, number, SUM(biomass_kgha) AS biomass_kgha
    ...
    INNER JOIN (
        ...
        jsonb_agg(
            ...
            ROUND(biomass_kgha_tg, 1))
        ) AS biomass_kgha_tg
        FROM (
            SELECT site_id, trophic_group, SUM(biomass_kgha) AS biomass_kgha_tg
            FROM fb_se_trophic_groups
...
LEFT JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            SELECT site_id, name, 
            ROUND(AVG(cat_percent), 1) as avg
            FROM (
                WITH cps AS (
                    SELECT 
                    sample_event.site_id,
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('summary_api', '0006_auto_20190829_2040'),
    ]
